...
router.get("/",function (req, res) {
  ...
  if (!minimumFilmDuration) return res.json(jeuModel.getAll());
  else {
    res.json(jeuModel.getOneByName(minimumFilmDuration));
  }
});
...
router.post("/",authorizeFromCookie,function (req, res) {
  ...
  if (
    ...
    (req.body.hasOwnProperty("first_release_date") && req.bodyfirst_release_date === 0) ||
